...
const Home = () => {
    ...
    const [cart] = useLocalStorage('cartDetail', {});
    ...
    useEffect(() => {
        fetchData('banner').then(setBanner);
        fetchData('barista').then(setBarista);
    }, []);
    ...
    return (
        <main>
            ...
            {ProductCart.length ? <section className='bottom-0 z-30 fixed w-full'>
                <HomeSlide
                    cart={cart}
                    pages='ordering/cart'
                    BtnName='View Cart' />
