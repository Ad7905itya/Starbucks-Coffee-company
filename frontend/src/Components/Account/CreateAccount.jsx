...
const CreateAccount = () => {
    ...
    return (
        <main>
            <Helmet>
                <title>Register | Tata Starbucks</title>
            </Helmet>
            <BreadcrumbNav items={[
                { label: 'Home', path: '/' },
                    ...
                    <div className='flex justify-center mt-14 mb-20'>
                        {SubmitBtn ? (
                            <Link to="/registration/verification">
                                <button
                                    onClick={() => setSubmitData(AccountData)}
                                    className='bg-[#00754a] hover:bg-[#1e3932] px-6 py-3 rounded-3xl w-[330px] font-bold text-white text-xs'>
                                    Continue
                                </button>
                            </Link>
                        ) : (
