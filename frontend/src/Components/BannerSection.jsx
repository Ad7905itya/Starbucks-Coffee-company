    ...
    return (
        <div className='flex items-center mx-2 h-full'>
            <div style={{ backgroundImage: `url(${Banner.image})` }} className={`bg-cover bg-no-repeat bg-center shadow-4xl rounded-xl w-full h-[90%] flex items-center relative cursor-pointer`}>
                ...
                <div className='flex flex-col items-start self-start gap-1 md:gap-3 mt-8 ml-3 text-white'>
                    <p className='text-xs'>{Banner.subTitle}</p>
                    <h2 className='text-sm lg:text-lg'>{Banner.title}</h2>
                    <p className='text-xs lg:text-base text-start'>{Banner.description}</p>
                </div>
                <div className={`absolute top-2 md:top-auto -right-3 md:right-5 bottom-2 flex items-end w-44 h-full`}>
